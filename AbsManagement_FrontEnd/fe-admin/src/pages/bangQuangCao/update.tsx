...
export function UpdateBangQuangCao(): JSX.Element {
    ...
    function getDiemDatQuangCaos(useInfo,idDiemDatQuangCao : any | null) {
        diemDatQuangCaoAPI
            ...
            .then((response) => {
                if(response && response.status === 200)
                {
                    ...
                    if(idDiemDatQuangCao){
                        const diaDiem = response.data.find(t=>t.id === idDiemDatQuangCao);
    ...
    useEffect(() => {
        ...
        getDiemDatQuangCaos(useInfo,bangQuangCao.idDiemDatQuangCao)
        form.setFieldsValue(bangQuangCao)
        getLoaiBangQuangCaos();
        form.setFieldValue('ngayBatDau',dayjs(bangQuangCao.ngayBatDau))
        form.setFieldValue('ngayHetHan',dayjs(bangQuangCao.ngayHetHan))
        const fileImages:UploadFile[] = [];
        bangQuangCao.danhSachHinhAnh.forEach((image,index)=>{
    ...
    useEffect(() => {
        if(!mapContainerRef.current || !bangQuangCao)
        {
            return;
        }
        ...
        map.current.on('load', () => {
            console.log("diembangquangcaos",diemDatQuangCaos)
    ...
    async function onSubmit(_model: CapNhatBangQuangCaoModel) {
        setLoading(true)
        if (_model &&  bangQuangCao) {
    ...
    return (
        ...
        <Suspense fallback={<PageLoading/>}>
            <PageContainer title={user?.role === "CanBoSo" ? "Cập nhật bảng quảng cáo" : "Cấp phép chỉnh sửa bảng quảng cáo"}>
                    ...
                    <Row gutter={[10,10]}>
                        <Col span={14}>
                            <Card title={<b>Thông tin bảng quảng cáo</b>} bordered={false}>
                                ...
                                rules={[{ required: true ,message: messageValidate.RequireTinhTrang}]}>
