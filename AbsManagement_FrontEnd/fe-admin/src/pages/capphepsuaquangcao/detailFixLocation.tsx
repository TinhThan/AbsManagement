...
const DetailFixLocation: FC = () => {
    ...
    async function getDetail(id: string) {
        ...
        await phieuChinhSuaAPI
            .ChiTietPhieuSuaDiemDat(parseInt(id))
            .then((response) => {
    ...
    useEffect(() => {
        ...
        const map = new mapboxgl.Map({
            ...
            center: [data?.diemDatQuangCao ? data?.diemDatQuangCao.danhSachViTri[0] : '106.681216', data?.diemDatQuangCao ? data?.diemDatQuangCao.danhSachViTri[1] : '10.76252'], // Ho Chi Minh City
            zoom: 15,
        });

        map.on('load', () => {
            new mapboxgl.Marker()
                .setLngLat([data?.diemDatQuangCao ? data?.diemDatQuangCao.danhSachViTri[0] : '106.681216', data?.diemDatQuangCao ? data?.diemDatQuangCao.danhSachViTri[1] : '10.76252'])
                .addTo(map);
    ...
    return (
        <Suspense fallback={<PageLoading />}>
            <PageContainer title="Chi tiết điểm đặt">
                        ...
                        <Row gutter={[10, 10]}>
                            <Col span={14}>
                                ...
                                >
                                    ...
                                    <Form.Item label='Địa chỉ'>
                                        <Input.TextArea value={data?.diemDatQuangCao.diaChi} readOnly rows={3} autoSize={{ minRows: 3, maxRows: 5 }} />
                                    </Form.Item>
                            ...
                            <Col span={10}>
                                <Space direction='vertical' style={{ width: '100%', marginBottom: '10px' }}>
                                    ...
                                    >
                                        {data && data.diemDatQuangCao.danhSachHinhAnh && data.diemDatQuangCao.danhSachHinhAnh.length > 0 ?
                                            <Image.PreviewGroup>
                                                <Space direction='vertical' size={0} style={{ marginBottom: '10px' }}>
                                                    {data.diemDatQuangCao.danhSachHinhAnh.length > 1 ? (
                                                        <Space size={5}>
                                                    ...
                                                    ) : <Image width={150} height={150} src={`${process.env.REACT_APP_BASE_API}Upload/image/${data.diemDatQuangCao.danhSachHinhAnh[0]}`} alt='avatar' />}
                                                </Space>
                                            </Image.PreviewGroup> :
                                            <Empty />}
                                    </Card>
...
export default DetailFixLocation