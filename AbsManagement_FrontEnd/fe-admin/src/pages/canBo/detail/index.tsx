...
import { getDistrictWithCode, getDistrictByCode, getWardByDistrictWithCode } from '../../../utils/getWard';
    ...
    >
      <Col>
        ...
        <Form.Item label={"Nơi công tác"}>
          <Input value={ canBo.noiCongTac.length > 0 ? canBo.noiCongTac.length > 1 ? `Phường ${getWardByDistrictWithCode(canBo.noiCongTac[0],canBo.noiCongTac[1]).name}, Quận ${getDistrictWithCode(canBo.noiCongTac[0]).name}`: `Quận ${getDistrictByCode(canBo.noiCongTac[0]).name}` : ''} readOnly />
        </Form.Item>
      </Col>
    </ModalDetail>
  );
